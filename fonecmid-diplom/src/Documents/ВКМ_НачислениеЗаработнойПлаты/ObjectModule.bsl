...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Строка Из НачисленияЗП Цикл
		...
		Если Строка.ВидРасчета = ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.ПремияФиксированная Тогда
...
Процедура СформироватьДвижения()
	...
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Сотрудник КАК Сотрудник,
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ВидРасчета КАК ВидРасчета,
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ДатаНачала КАК ДатаНачала,
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ДатаОкончания КАК ДатаОкончания,
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ГрафикРабот КАК ГрафикРаботы
	|ПОМЕСТИТЬ ВТ_Док
	|ИЗ
	|	Документ.ВКМ_НачислениеЗаработнойПлаты.НачисленияЗП КАК ВКМ_НачислениеЗаработнойПлатыНачисленияЗП
	|ГДЕ
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Пока Результат.Следующий() Цикл
		...
		Если ТипЗнч(Результат.ВидРасчета) = Тип("ПланВидовРасчетаСсылка.ВКМ_ОсновныеНачисления") Тогда
			...
			Движение.ГрафикРабот = Результат.ГрафикРаботы;
			
			// проверка
				Если НЕ ЗначениеЗаполнено(Движение.ГрафикРабот) Тогда
    				// Подставляем график из шапки или по умолчанию, если в ТЧ он не заполнен для отпуска
    				Движение.ГрафикРабот = Справочники.ВКМ_ГрафикиРабот.НайтиПоНаименованию("5-2");
				КонецЕсли;
...
Процедура СформироватьДвиженияУдержания()
	...
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Ссылка = &Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Сотрудник";
...
Процедура ДвиженияУчетОтпускаСотрудников() 
	...
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Сотрудник КАК Сотрудник,
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ДатаОкончания КАК ДатаОкончания,
	|	РАЗНОСТЬДАТ(ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ДатаНачала,
	|		ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ДатаОкончания, ДЕНЬ) + 1 КАК КоличествоДней
	|ИЗ
	|	Документ.ВКМ_НачислениеЗаработнойПлаты.НачисленияЗП КАК ВКМ_НачислениеЗаработнойПлатыНачисленияЗП
	|ГДЕ
	|	ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.Ссылка = &Ссылка
	|	И ВКМ_НачислениеЗаработнойПлатыНачисленияЗП.ВидРасчета = &ВидРасчета";
